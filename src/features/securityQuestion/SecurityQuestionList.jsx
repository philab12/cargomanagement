...
import { selectAllSecurityQuestions, useGetSecurityQuestionsQuery, useDeleteSecurityQuestionMutation } from  './securityQuestionApiSlice';
...
function SecurityQuestionList() {
    ...
    const [deleteSecurityQuestion,{isLoading:delLoading}] = useDeleteSecurityQuestionMutation();
    const selectPagee = useSelector(selectPage);
    const [selectionModel,setSelectionModel] = useState([]);

    const {isLoading, isSuccess, isError, error } = useGetSecurityQuestionsQuery('securityQuestionList' /*or null*/, {
        pollingInterval: 15000,
        refetchOnFocus: true,
        refetchOnMountOrArgChange: true
    })

    const data = useSelector(selectAllSecurityQuestions);
...
const handleEdit = (id) => {
    ...
    if(selectPagee !== MODALNAMES.SECURITYQUESTIONMODAL){
        dispatch(setActionPage({isEdit:true, page:MODALNAMES.SECURITYQUESTIONMODAL, id}))
        }
}
    ...
    const buttons = [
        {
          ...
          onClick: async () => {
            ...
            try{
                await deleteSecurityQuestion(id).unwrap()
                successNotification("This Security Question  Deleted Successfully");
             } catch(error){
                errorNotification("This Security Question Could Not Be Deleted");
             }
          ...
          //  dispatch(setActionPage({isEdit:true, page: MPages.HEALTHCARE, id:currentRow.id}))
           // dispatch(setActionPage({isEdit:true, page:MODALNAMES.COURIERTYPEMODAL, id:currentRow.id}))
            ...
            //     }
                ...
                if(selectPagee !== MODALNAMES.SECURITYQUESTIONMODAL){
                    dispatch(setActionPage({isEdit:true, page:MODALNAMES.SECURITYQUESTIONMODAL, id:currentRow.id}))
                    }
            ...
            const buttons = [
                {
                  ...
                  onClick: async () => {
                    ...
                    try{
                        await deleteSecurityQuestion(currentRow.id).unwrap()
                        successNotification("This Security Question  Deleted Successfully");
                     } catch(error){
                        errorNotification("This Security Question Could Not Be Deleted");
                     }
        ...
        {/* {delLoading ? <p>Loading...</p> : null}
            <Table hoverable striped>
                <Table.Head>
                    ...
                    <Table.HeadCell>Security Question</Table.HeadCell>
