...
export default function ReceiveCourierModal() {
  ...
  useEffect(() => {
    if(isEdit === true && selectPagee === MODALNAMES.RECEIVECOURIERMODAL){
   setOpenModal(true) ;
   }
  ...
  return (
    <>
      ...
      <Modal show={openModal} onClose={handleCloseModal} size="4xl">
        <Modal.Header>{"Receive Courier"}</Modal.Header>
        <Modal.Body>
          <ReceiveCourierForm handleCloseModal={handleCloseModal} />
        </Modal.Body>
        <Modal.Footer>
