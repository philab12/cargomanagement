...
import ExtraChargeForm from "../features/extraCharge/ExtraChargeForm"
...
export default function CourierPriceModal() {
  ...
  useEffect(() => {
   ...
   if(isEdit === true && selectPagee === MODALNAMES.EXTRACHARGE ){
    setOpenModal(true) ;
    }
  ...
  return (
    <>
      ...
      <Modal show={openModal} onClose={handleCloseModal} size="4xl">
        <Modal.Header>{!isEdit ? "New Extra Charge" : "Edit Extra Charge"}</Modal.Header>
        <Modal.Body>
          <ExtraChargeForm  handleCloseModal={handleCloseModal} />
        </Modal.Body>
        <Modal.Footer>
